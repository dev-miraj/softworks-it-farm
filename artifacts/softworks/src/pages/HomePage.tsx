        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 reveal-stagger">
          {capabilities.map((cap, i) => {
            ...
            return (
              <div
                key={cap.title}
                className="relative group cursor-pointer rounded-2xl p-5 sm:p-6 border border-border/50 bg-card/50 hover:bg-card hover:border-primary/20 hover:shadow-xl hover:shadow-primary/5 transition-all duration-500"
                style={{ animationDelay: `${i * 80}ms` }}
              >
                <div className={`w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br ${cap.gradient} p-0.5 mb-4 sm:mb-5 shadow-lg`}>
      ...
      <section className="py-16 sm:py-24 max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-16 items-center">
          <div className="reveal-left">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-secondary/30 bg-secondary/10 text-secondary text-sm font-medium mb-5 sm:mb-6">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 reveal-right">
            {metrics.map((metric) => (
              <div
                key={metric.label}
                className="relative rounded-2xl p-5 sm:p-6 text-center overflow-hidden group hover:scale-105 transition-transform duration-300 border border-border bg-card"
              >
                ...
                <div className="text-[10px] sm:text-xs text-muted-foreground font-medium tracking-wide uppercase">
                  {metric.label}
                </div>
              </div>
            ))}
      ...
      {activeTestimonials.length > 0 && (
        <section className="py-16 sm:py-20 border-y border-border bg-muted/30">
          <div className="max-w-6xl mx-auto px-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-5 reveal-stagger">
              {activeTestimonials.slice(0, 3).map((t) => (
                <div key={t.id} className="rounded-2xl p-5 sm:p-6 border border-border bg-card hover:border-primary/20 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 group">
